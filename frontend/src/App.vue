<template>
  <div v-if="v==='login'" class="login-page">
    <div class="login-card">
      <svg width="40" height="40" viewBox="0 0 48 48"><circle cx="24" cy="24" r="22" fill="none" stroke="#002EA6" stroke-width="2.5"/><path d="M16 20 L24 14 L32 20 L28 20 L28 34 L20 34 L20 20 Z" fill="#FFE76F"/></svg>
      <h1>A21 船舶故障诊断</h1>
  ...
  <div v-else class="app">
    ...
    <div class="main">
      ...
      <HistoryPanel v-if="v==='history'" @select="onH" />
    </div>
  </div>
</template>
<script setup>
...
if(localStorage.getItem('a21_token')){authAPI.me(localStorage.getItem('a21_token')).catch(()=>{const a=localStorage.getItem('a21_auto');if(a){const[x,y]=a.split(':');authAPI.login({username:x,password:y}).then(({data})=>{localStorage.setItem('a21_token',data.token);localStorage.setItem('a21_user',JSON.stringify(data.user));v.value='chat'}).catch(()=>{v.value='login'})}else{v.value='login'}})}
async function login(){if(!u.value||!p.value){er.value='请输入用户名和密码';return};ld.value=true;er.value='';try{const{data}=await authAPI.login({username:u.value,password:p.value});localStorage.setItem('a21_token',data.token);localStorage.setItem('a21_user',JSON.stringify(data.user));v.value='chat'}catch(e){er.value='登录失败'}finally{ld.value=false}}
function onN(n){};function onH(s){v.value='chat'}
</script>
<style>
...
.app{display:flex;height:100vh;background:#FAFAFA}.main{flex:1;overflow:hidden;display:flex;flex-direction:column}
...
.login-card svg{margin-bottom:20px}.login-card h1{font-size:20px;color:#1A1A2E;margin-bottom:28px;font-weight:600}
