<template>
  <div class="chat">
    ...
    <div class="ia">
      <div class="ir">
        <textarea v-model="tx" placeholder="输入故障现象或问题..." @keydown.enter.exact.prevent="send" :disabled="st" rows="2"/>
        <button @click="startVoice" class="vb" title="语音输入">🎤</button>
      ...
      <label class="ub" title="上传文档">📄<input type="file" accept=".docx,.pdf,.txt" @change="upload" hidden/></label>
    </div>
...
const msgs=ref([]),tx=ref(''),st=ref(false),stx=ref(''),ml=ref(null),va=ref(false),vt=ref('')
function esc(s){return s.replace(/</g,'&lt;').replace(/>/g,'&gt;')}
function md(s){return s.replace(/\*\*(.*?)\*\*/g,'<strong>$1</strong>').replace(/### (.*)/g,'<h3>$1</h3>').replace(/\n\n/g,'<br><br>').replace(/\n/g,'<br>').replace(/\[(\d+)\]/g,'<sup class="c">[$1]</sup>').replace(/^- (.*)/gm,'<li>$1</li>')}
async function send(){if(!tx.value.trim()||st.value)return;const q=tx.value.trim();tx.value='';msgs.value.push({id:Date.now(),role:'user',content:q});st.value=true;stx.value='';await nextTick();scroll()
  try{const r=await askStream(q,{mode:'chat'});const d=new TextDecoder();let b=''
...
async function startVoice(){
  try{
    va.value=true;vt.value=''
    mediaStream=await navigator.mediaDevices.getUserMedia({audio:{sampleRate:16000,channelCount:1}})
    ws=new WebSocket('ws://127.0.0.1:8765')
    ws.onmessage=e=>{try{const d=JSON.parse(e.data);if(d.type==='partial')vt.value=d.text;else if(d.type==='final'&&d.text)vt.value+=' '+d.text}catch{}}
    ws.onerror=()=>stopVoice()
    await new Promise(r=>ws.onopen=r)
    audioCtx=new AudioContext({sampleRate:16000})
    const src=audioCtx.createMediaStreamSource(mediaStream)
    processor=audioCtx.createScriptProcessor(4096,1,1)
    processor.onaudioprocess=e=>{if(ws?.readyState===WebSocket.OPEN){const d=e.inputBuffer.getChannelData(0);const buf=new Int16Array(d.length);for(let i=0;i<d.length;i++)buf[i]=Math.max(-1,Math.min(1,d[i]))*0x7FFF;ws.send(buf.buffer)}}
    src.connect(processor);processor.connect(audioCtx.destination)
  }catch(e){stopVoice();va.value=false}
}
function stopVoice(){
  ...
  if(vt.value.trim())tx.value=vt.value.trim()
  va.value=false
}
async function upload(e){const f=e.target.files[0];if(!f)return;const fd=new FormData();fd.append('file',f);try{const{data}=await api.post('/parse',fd,{headers:{'Content-Type':'multipart/form-data'}});tx.value=`请分析以下文档:\n${data.candidates?.map(c=>`- ${c.type}: ${c.name}`).join('\n')||'无结果'}`}catch{}}
async function rate(msg,rating){msg._r=msg._r===rating?0:rating;const q=msgs.value.find(m=>m.role==='user'&&m.id<msg.id)?.content||'';try{await feedbackAPI.submit({message_id:msg.id+'',rating,question:q,answer_text:msg.content,retrieved_chunks:'[]'})}catch{}}
</script>
<style scoped>
...
.msg{display:flex;gap:12px;margin-bottom:20px}.msg.user{flex-direction:row-reverse}
...
:deep(h3){font-size:16px;margin:8px 0 4px}:deep(strong){font-weight:600}:deep(li){margin:3px 0 3px 16px}:deep(.c){color:#06B6D4;font-size:11px;cursor:pointer;margin:0 2px}
.ia{padding:14px 20px;border-top:1px solid #F0F0F0;background:#fff;display:flex;align-items:flex-end;gap:10px}
.ir{display:flex;gap:8px;align-items:flex-end;flex:1}
textarea{flex:1;padding:10px 14px;border:1.5px solid #E5E7EB;border-radius:12px;resize:none;outline:none;font-size:15px;line-height:1.5;font-family:inherit}textarea:focus{border-color:#002EA6;box-shadow:0 0 0 3px rgba(0,46,166,0.1)}
.sb{width:40px;height:40px;border:none;border-radius:10px;background:#002EA6;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0}.sb:disabled{background:#D1D5DB;cursor:not-allowed}
.vb{width:40px;height:40px;border:1.5px solid #E5E7EB;border-radius:10px;background:#fff;cursor:pointer;font-size:18px;flex-shrink:0}.vb:hover{background:#F0F4FF}
...
.vbx p{font-size:16px;color:#333;margin-bottom:4px}.vt{font-size:18px!important;color:#002EA6!important;font-weight:500;min-height:28px;margin:12px 0!important}
.vs{margin-top:16px;padding:10px 32px;background:#002EA6;color:#fff;border:none;border-radius:10px;cursor:pointer;font-size:15px;font-weight:500}
</style>
